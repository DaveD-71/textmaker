...
Glossary and teacher-answer-book already existed per book (Phase 5/6 deliverables built during the QA-fix pass, `qa-todo.md` item 6). Student edition built this phase: `drafts/output/<Book Title> - Student Edition.md`, one file per book — front matter (`00_How_This_Resource_Is_Organized.md`'s content) followed by all 20 topics in strict Part.Topic order with `# Part N: Title` divider headings inserted before each Part's first topic. Verified: exactly 20 `## X.Y.` topic headings and 4 `# Part N` headings per book, correct order, no truncation, no stray/duplicate H1s.

**Extended same day: DOCX build using the IR project's actual reference documents.** User provided the three "Let's Talk: Investor Relations" output DOCX files (`Investor Relations Resource - Articles.docx`, `- Glossary.docx`, `IR Teacher Answer Book.docx`) as the house-style reference for Pandoc's `--reference-doc`. Built all 6 deliverables (student edition, glossary, teacher answer book, x2 books) via `textmaker.cmd markdown-to-docx`.

Required fixes to the shared `scripts/postprocess_docx.py`/`scripts/cli.py` pipeline (it was built for the Speaking with PowerPoint project and had several SWP-specific behaviors that unconditionally ran regardless of reference doc):
- `apply_list_styles` and `strip_literal_alpha_markers` hard-crashed when the reference DOCX had no `PS Bullet List`/`PS Numbered List`/`List Number 3` styles (the IR docs use none) — changed to a graceful skip-with-warning instead of `RuntimeError`.
- `remove_pandoc_generated_styles` hard-required a `Body Text Char` character style that the IR docs don't define — made that half of the pass optional.
- `apply_presentation_page_setup` unconditionally overwrote page margins/mirroring with SWP-specific values regardless of reference doc — added `--no-presentation-page-setup` CLI flag to skip it.
- `disable_heading_style_page_breaks` unconditionally stripped `pageBreakBefore` from Heading 1/2 styles in favor of explicit section breaks (right for SWP, wrong for the IR pattern, which uses the style flag directly) — added `--keep-heading-page-breaks` CLI flag to skip it.

New LTF-specific tooling in `scripts_local/` (not shared-pipeline changes, since this logic is specific to matching the IR document's exact structure):
- `build_ltf_student_edition_md.py` — markdown-level transform (build-output only, does not touch the tracked merged `.md` or per-article sources): strips the `**Recycled terms:**` line and `**New terms:**` label from Vocabulary Focus, leaving one plain unlabeled term list (confirmed by direct inspection this is the IR reference's actual final format); converts each Source Note's trailing bare URL into a proper markdown link so Pandoc emits a real `w:hyperlink`.
- `apply_ir_content_styles.py` — position-based paragraph restyling (Reading body -> `article_body_text`, first paragraph after Vocabulary Focus heading -> `First Paragraph`, Reading/Discussion Questions and Source Notes -> `Compact`), since the LTF markdown has no fenced Div classes to drive Pandoc's own `style_bridge.lua` mapping automatically.
- `apply_ir_teacher_book_styles.py` — same idea for the teacher-answer-book's Aim/Target vocabulary/Confidentiality note (-> `First Paragraph`) and numbered answer items (-> `Compact`).
- `add_two_column_reading_sections.py` — inserts the per-topic section-break pattern discovered by directly inspecting the IR Articles DOCX's OOXML: a `CONTINUOUS` single-column section holding the topic title + Goal, closed right after the "Reading" heading; a `CONTINUOUS` 2-column section spanning just the Reading body paragraphs, closed right after the last one; single-column resumes for Vocabulary Focus onward. Hard-asserts the expected topic count and exact section-break count before saving (per the process guide's own warning about silent section-break corruption).
- `fix_part_heading_page_breaks.py` — the IR reference's Heading 1 style also carries `pageBreakBefore`, but in the actual IR document a Part heading and its first topic land on the same page; explicitly clears the paragraph-level flag on every Heading 1 paragraph (topics still break via the Heading 2 style flag).
- `add_page_number_footer.py` — replicates the IR doc's centered "p. [PAGE]" footer, starting at the first content section (the title-page/front-matter section stays footer-free, matching the reference).
- `style_glossary_docx.py` — the IR Glossary reference is single-column front matter followed by ONE continuous 2-column section spanning every entry through the last; restyles every entry paragraph to `glossary_para` (Noto Serif Light, justified) and wraps them in that 2-column section.
- Also caught and fixed: a `---` horizontal-rule Part divider in the merged markdown was being converted into a literal hard page break by the pipeline's existing `pagebreak.lua` filter, producing a wasted near-blank "Part N" page — fixed with the existing `--ignore-horizontal-rules` CLI flag (drops the marker before Pandoc sees it) rather than any new code.

Verified per topic: Reading section fits one A4 page in 2-column layout (confirmed visually via PDF render, both books); topics start on fresh pages; Source Note URLs are real hyperlinks; Vocabulary Focus shows the plain unlabeled format. Not yet addressed: whether the full topic (Goal through Source Notes, not just the Reading) needs to fit one page — house-style's literal wording is about the Reading specifically, and the current build satisfies that; the entire-topic question was raised but not resolved before the user paused this thread to continue manually.
