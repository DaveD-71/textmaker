...
4. **Timeline Verification:** Determine whether the revision affects the processing timeline; update if necessary.
...
**Discussion:** The improved version contains no exceptions. Why might a real procedure need at least one exception clause? What would you add here?
...
1. *<u>Do</u> the initial review before forwarding the file.*
2. *<u>Look at</u> the changes made in the revised submission.*
3. *<u>Deal with</u> the applicant's request within two working days.*
4. *<u>Make sure</u> the record is updated before the file is forwarded.*
5. *<u>Check</u> whether the timeline needs to be changed.*

**Useful verbs:** *conduct, examine, verify, process, ensure, determine, cross-check, update, confirm*
...
Choose one of the following scenarios and write a complete internal procedure.

1. The procedure for requesting an extension to a submission deadline
2. The procedure for transferring a case file to another office
3. The procedure for processing an applicant's appeal against a procedural decision

Your procedure must include:

- 6–9 clearly numbered and labelled steps
...
2. Are conditions signalled clearly (e.g., *if*, *unless*, *where X applies*)?
...
Write a **200–230 word** procedure for an internal or interagency process of your choice. Your procedure must include:

- 7–10 clearly numbered and labelled steps
- at least one conditional branch
- at least one exception
...
Suggested topics:
- Processing a translation discrepancy in a submitted document
- Coordinating a joint review between two offices
- Handling a submission that arrives after the official deadline
...
> **Scenario:** Your office has updated the procedure for reviewing applicant submissions. The main change is that the initial review stage, which previously required sign-off from two officers, now requires only one. This change takes effect on 1 November. Staff in your office and in three partner offices need to be informed.
...
1. Your team has changed the internal workflow for confirming translations — the confirmation step now takes place before filing, rather than after.
2. A partner office has updated the terminology used in all procedural documents — two key terms have been replaced with new ones throughout.
3. Your agency has introduced a new requirement: all external correspondence must be copied to the coordinating office before dispatch.
...
Choose one scenario and write a complete change management notice.

1. Announcement of a major procedural update affecting multiple offices
2. Notification of a policy adjustment that affects how applicant documents are handled
3. Revision of an interagency coordination workflow, reducing the number of required sign-offs
...
Suggested topics:
- A change to the process for handling urgent submissions
- An update to the format required for internal reports
- A new approval requirement for outgoing correspondence to foreign offices
...
**Discussion:** Why is it important to use neutral language when explaining a delay or problem to a partner office? What can go wrong when the tone becomes evaluative or accusatory?
...
Choose one scenario and write a structured explanation of the issue.

1. A report cannot be completed because of missing data from one office and an inconsistency in the guidelines
2. An interagency review has stalled because two offices have submitted conflicting information
3. A translation correction has created new questions that need to be resolved before the file can proceed
...
Choose one of the following scenarios and write a complete explanation of the issue.

1. Multi-office coordination delays affecting a processing deadline
2. Conflicting instructions in newly issued guidelines
3. An incomplete submission that has created downstream problems for your office

**Your constraint:** Every paragraph must contain at least one specific detail. You may not write a general statement — such as *there are some problems* or *this is complicated* — without immediately following it with a concrete example or piece of information.
...
Suggested topics:
- A delay in finalising a joint report
- An inconsistency between two versions of an interagency guideline
- A processing backlog caused by a combination of staffing and procedural issues
...
**Task:** Using the data in the table, write the *Decisions / Action Items* and *Next Steps* sections of an executive meeting summary. You do not need to invent what was discussed — focus on accurately reporting what the table tells you about outcomes and outstanding items. Write in formal, complete sentences.
...
Choose one scenario and write a complete executive-level meeting summary.
