...
- **As of 2026-09-14, the SWP-specific postprocessing behaviors below are opt-in, not the pipeline default** (another project, Let's Talk Finance, uses the same `markdown-to-docx` command against a non-SWP reference DOCX, so SWP-only behavior moved behind explicit flags rather than needing every other project to opt out of it). SWP builds must now also pass `--swp-presentation-page-setup` (the A4 mirror-margin page setup below), `--swp-disable-heading-page-breaks` (strips `pageBreakBefore` from Heading 1/2 in favor of explicit section breaks), and `--swp-list-styles` (remaps bullet/numbered/alphabetic lists to `PS Bullet List` / `PS Numbered List` / `PS Numbered List 2`) alongside `--swp-style-tags`.
- Page setup is enforced after Pandoc conversion (only when `--swp-presentation-page-setup` is passed): A4 portrait, mirror margins, top `2.5 cm`, bottom `2.0 cm`, inside `4.0 cm`, outside `2.5 cm`, header from edge `1.0 cm`, footer from edge `0.8 cm`, odd/even headers and footers enabled, odd-page header/footer alignment right, even-page header/footer alignment left.
...
- Current uncommitted Speaking with PowerPoint edits:
  ...
  - `revision/control/presentations-component-library.md`: upgraded from a component inventory to a BR2e-style production specification with required fields, detailed component-family specs, implementation matrix, and build-ready definition.
  - `revision/control/presentations_style.yaml` and `revision/control/presentations_style.docx`: switched from the earlier Arial/Palatino-style direction to a Noto Serif/Noto Sans/Noto Sans Display system, Wada-derived formula-tracked palette, left-aligned headings, more generous heading spacing, and richer Word-native table styles refined through `scripts/refine_presentations_reference_docx.py`.
  - Latest `presentations_style.docx` repair: production margins are top `20 mm`, bottom `25 mm`, left `40 mm`, right `25 mm`; odd/even page-number footers are enabled with right-aligned page numbers; all heading styles suppress auto-hyphenation; unit title/display size is `28 pt`; shaded/fill styles have more left/right inset; list-block spacing is tighter; table header rows use light text on dark fills with no space after.
