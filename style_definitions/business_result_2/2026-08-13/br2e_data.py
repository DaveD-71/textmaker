...
#
# Version history:
#   4.1  2026-08-13  Added A4 scaling system (SCALE, TARGET_W/H, scaled_*() helpers)
#   4.0  2026-05-10  Rebuilt as generated system from authoritative data module
#                    Replaced all manual HTML edits with single-source-of-truth approach
#   3.5  2026-03-23  Full font audit; pixel-sampled level accent colours;
#                    mm + percentage measurement columns; complete font inventory
#   3.0  2026-03-16  All values vector-confirmed; Viewpoint band corrected;
#                    page geometry and spacing fully measured
# ============================================================

VERSION      = "4.1"
VERSION_DATE = "2026-08-13"
VERSION_NOTE = "A4 scaling added — all output values proportionally scaled from BR2e source"

# ── Source page dimensions (BR2e original) ───────────────────
SOURCE_W = 174.6   # mm — original content area width (confirmed from vector)
SOURCE_H = 247.0   # mm — original content area height (confirmed from vector)

# ── Target page dimensions ───────────────────────────────────
# Set these to your target page size.
# Margins are not included here — these are the full page dimensions.
# The scale factor is derived automatically; do not set it manually.
TARGET_W = 210.0   # mm — A4 width
TARGET_H = 297.0   # mm — A4 height

# ── Scale factor ─────────────────────────────────────────────
# Derived from the ratio of target to source dimensions.
# Width and height ratios are nearly identical (1.2027 vs 1.2024),
# so a single uniform factor is used — no distortion occurs.
# If you change TARGET_W/TARGET_H, this updates automatically.
_scale_w = TARGET_W / SOURCE_W   # 210.0 / 174.6 = 1.2027
_scale_h = TARGET_H / SOURCE_H   # 297.0 / 247.0 = 1.2024
SCALE = (_scale_w + _scale_h) / 2  # average = 1.2026 — uniform, no distortion

# Working page dimensions (source scaled to target)
PAGE_W = SOURCE_W * SCALE   # ≈ 210.0mm
PAGE_H = SOURCE_H * SCALE   # ≈ 297.0mm

def pw(mm_val):
    """Percentage of target page width, 2 decimal places."""
    return f"{mm_val / PAGE_W * 100:.2f}%"

def ph(mm_val):
    """Percentage of target page height, 2 decimal places."""
...
def spt(val):
    """Scale a pt value and format with suffix."""
    return f"{s(val):.2f}pt"

def mm(val):
    """Format mm value with suffix (unscaled — for raw values)."""
    return f"{val}mm"

def pt(val):
    """Format pt value (unscaled — for raw/exact values)."""
    return f"{val}pt"
...
def scaled_type_scale():
    """Return TYPE_SCALE with sizes scaled to target page dimensions.
    Use this instead of TYPE_SCALE directly when generating output."""
    return [
...
def scaled_geometry():
    """Return GEOMETRY with mm values scaled to target page dimensions."""
    return [
...
def scaled_positions():
    """Return POSITIONS with mm values scaled to target page dimensions."""
    result = []
...
def scaled_gaps():
    """Return GAPS with mm values scaled to target page dimensions."""
    return [
        (from_to,
...
def scaled_x_positions():
    """Return X_POSITIONS with mm values scaled to target page dimensions."""
    return [
        (label,
...
def scaled_leading():
    """Return LEADING with mm values scaled to target page dimensions."""
    return [
        (role,
...
def scaled_word_styles():
    """Return WORD_STYLES with point sizes scaled to target page dimensions."""
    return [
