...
2. **Purpose & Objectives**
   ...
   - First EN line → `details.purpose.en`.
   - First JA line → `details.purpose.ja`.
   - Remaining EN lines → `details.objectives.en`.
   - Remaining JA lines → `details.objectives.ja`.
...
2. **Lists**
   ...
   - Convert list items to dash-prefixed lines (`-`) **only** when the source line already begins with a list marker (`-`, `*`, `1.`, `a)`).
   - Do **not** infer list items based solely on a preceding colon (`:` or `：`). Many sentences end with colons without intending a list, and this was causing false list formatting.
   - If the DOCM uses a visual list without explicit markers, preserve the lines as plain text unless the list markers are present in the extracted text.
3. **Quotes**
