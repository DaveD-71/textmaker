...
**Cover files updated via Word COM**, per user instruction to edit the DOCX directly rather than regenerate anything. Found both cover files (`Let's Talk Finance 1 - Cover.docx`, `Let's Talk Finance 2 - Cover.docx`) at `books/Let's Talk Finance/cover/` were a front+back cover spread (16.53"x11.69" landscape, front cover + spine + back cover in one document) with the back-cover blurb living inside a floating text box (Word Shape, not part of the main document body — `doc.Content.Find` does not search inside these, had to enumerate `doc.Shapes` directly and use each shape's own `TextFrame.TextRange`). Both files were found to be identical, unedited duplicates, both still labelled "Let's Talk Finance 1" and both using the old topic-template description (no mention of the current 6-part structure: Goal, Reading, Vocabulary Focus, 5 Reading Questions, 5 Discussion Questions, Source Notes).

**Fixed:** Book 2's cover now correctly reads "Let's Talk Finance 2" with its own description; both books' back-cover blurbs updated to accurately describe the current article structure (vocabulary focus, reading-comprehension questions, discussion questions, numbered source notes) instead of the old generic "reading passages followed by discussion questions." Kept the existing "Upper Intermediate+" level framing rather than correcting it to the house-style B1+/B2 target, per user's own textinspector.com CEFR readings from earlier in the project showing the actual text reads higher (C1+) than the nominal target — so "Upper Intermediate+" is the more honest cover claim, not an overstatement to correct.

**Overflow caught and fixed before finalizing:** the first draft of the new blurb text (with specific topic-name examples for each book) was longer than the original and overflowed the fixed-size text box, causing the last 1-2 lines to run behind the logo graphic — caught by rendering both covers to PNG via poppler and looking at them, not just checking the text was replaced. Trimmed both blurbs back down to close to the original's length (dropped the specific topic-name examples, kept the accurate structural description) and re-verified visually that both now fit completely with no overflow.

**Technical note for future cover edits:** Word's `Range.Find.Execute` via COM has a ~255-character search-string limit (throws "String parameter too long") and does not search inside floating text-box shapes via `doc.Content.Find` at all. The working pattern: locate the shape via `doc.Shapes` iteration checking `TextFrame.HasText` and a short anchor substring in `TextFrame.TextRange.Text`, then use plain .NET string `IndexOf` on that Text property to find exact character offsets, then use `TextFrame.TextRange.Duplicate` + `SetRange(start, end)` + `.Text = newValue` to replace a sub-range in place — this preserves the surrounding runs' formatting (bold title, etc.) instead of collapsing it the way setting the whole `TextRange.Text` at once would.

## 2026-09-14 (final) - Corrected section-break pattern; fixed Book A 4.4 page overflow; confirmed Book A's 20 topics match the original

**Section-break pattern corrected, both books.** User inspected the DOCX and corrected a misreading from earlier in the day: the break before each topic's 2-column Reading section should be `continuous`, and the break after it (going into Vocabulary Focus) should be `nextPage` — the opposite of what the build had. Verified this against the IR reference directly this time using `python-docx`'s own `section.start_type`/`TextColumns.Count` accessors rather than raw XML attribute reads (which had been a source of confusion earlier). Since the user had already made manual edits to both student-edition DOCX files, fixed this via direct Word COM automation (`PageSetup.SectionStart`) on the existing files rather than regenerating from the pipeline, preserving those manual edits. Fixed 19 of 20 section boundaries in each book (the 20th, topic 4.5/final topic, was already correct as the document's own terminal section).

**Book A topic 4.4 (Corporate Governance) page overflow fixed.** The Reading's last paragraph was spilling onto a second page at 495 words. Checked the teacher-answer-book's Reading/Discussion answers first to identify every load-bearing fact and exact phrase the article's wording is depended on (e.g. "credited as one trigger" is directly analyzed in Discussion answer 3; "owners-elect-the-board mechanism" is quoted in Discussion answer 4) before trimming, then tightened two sentences of scene-setting/framing prose that no question or answer referenced, cutting 495 to 472 words. Verified after trimming: citation markers still match Source Notes, all 4 recycled terms present, and all 10 exact load-bearing phrases identified from the teacher-book cross-check are still present verbatim. Applied the identical edit to all 4 places the text exists: the source article `.md`, the merged student-edition `.md`, the build-transformed `.md`, and the DOCX itself (via Word COM find/replace, since the user had already manually edited the DOCX and did not want it regenerated).

**Confirmed Book A's 20 topics match the original book's 20 topics.** User asked whether the rewrite followed the original project goal (rewrite the same 20 topics to the IR standard, then add a second book of 20 new topics) by pointing at the actual original `Let's Talk Finance.docx` file. Read it directly and extracted all 20 `Article Title` paragraphs, then matched every one against Book A's current 20 topics by content. All 20 matched one-to-one, with only trivial wording differences (e.g. "&" vs "and", "Cooperation" vs "International Cooperation") — confirmed no topics were dropped, substituted, or added.
