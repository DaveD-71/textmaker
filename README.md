...
- `--no-semantic-formatting`: skip the semantic postprocessing layer that applies custom textbook styles and callout blocks from the reference DOCX.
...
- After Pandoc conversion, `postprocess-docx` applies a semantic formatting layer by default:
  - `## Unit ...` headings become `Unit Tile` Quick Parts.
  - weak/original model paragraphs receive `Block Text Bad`.
  - revised/improved model paragraphs receive `Block Text Good`.
  - explicit word-count prompts receive `Homework Words`.
  - short post-list prompts such as `Then discuss:`, `Reflect:`, and `Example:` receive `After List`.
...
- Use `--no-semantic-formatting` to disable that layer for troubleshooting or comparison runs.
