...
- Status: `active`
- Scope: project/goals, project/methodology
- **Speaking with PowerPoint is ON HOLD** (user's explicit instruction this session). All work below is the new Let's Talk Finance track.
- **Two books, both to be produced to the standard of the completed "Let's Talk: Investor Relations" project** at `\\prod-fs-gen01\...\Dobson（在宅）\02. Clients\Marubeni\Ishida, Tetsuya\materials\drafts\` (a separate repo; its `Book_Draft_And_Edit_Process_Guide.md` is the reusable playbook, copied into this repo as `books/_lets-talk-finance-shared/process-guide.md`):
  - **Book A — `books/Let's Talk Finance/`** — a from-scratch rebuild of the existing 20-topic finance discussion resource. The pre-existing `Let's Talk Finance.docx` was a ChatGPT first draft with no citations, no vocabulary system, no level control, 2 topics carrying the wrong reading text (old topic 4 Fintech = market-fragmentation filler; old topic 5 AML = AI-in-finance filler) and 1 malformed sentence (old topic 6 Data Privacy: "The of data privacy..."). The 20 subjects are kept; the prose is discarded. Old material archived under `books/Let's Talk Finance/_archive/`.
  - **Book B — `books/Let's Talk Finance 2/`** — 20 NEW topics (approved list in `books/Let's Talk Finance/PROJECT-PLAN.md` §3). Personal-finance / corporate-finance / market-mechanics angle; companion volume to Book A, not overlapping.
- **The single controlling document is `books/Let's Talk Finance/PROJECT-PLAN.md`** (path since renamed to `books/Let's Talk Finance 1/PROJECT-PLAN.md` — see 2026-09-17 journal entry) — scope, all 8 resolved decisions (D1–D8), the 8-phase production process, both full topic lists with 4-Part groupings, and the reference index. Read it first when resuming.
- **Shared control layer `books/_lets-talk-finance-shared/`:** `house-style.md` (the authority on level, framing, paragraph rules, currency style, page layout), `article-checklist.md` (per-article minimum gate — run on every article), `topic-ledger.md` (all 40 topics + overlap-watch + a cross-topic primary-evidence register to prevent reuse), `process-guide.md`, `shared-term-bank.md`, `research-briefs/` (per-topic fact/source briefs from background agents).
- **Per-book control (`<book>/drafts/control/`):** `project-plan.md`, `qa-checklist-full.md` (whole-book Phase-5 audit, adapted from the IR project's), `vocabulary-map.md` (New/Recycled terms per topic — mostly empty, fill as drafting proceeds), `company-and-geography-audit.md`, `factual-data-insertion-guide.md`.
- **Key resolved decisions (full text in PROJECT-PLAN.md §2):**
  - **D1 level = CEFR B1+/B2** — one step easier than IR's B2–B2+. Governs sentence length (mostly 15–25 words, no 3-clause sentences), connectives, term introduction (define on first use), paragraph size. Learner = an educated adult **based in Japan**, primarily Japanese L1.
  - **D1 also — the Japanese vantage-point rule** (`house-style.md` §1a). Got this WRONG twice in the prototype pass, then corrected: (1) do NOT write from a non-Japanese default with Japan as an aside — a prototype line "People often ask why central banks do not aim for zero inflation" assumes a reader whose normal experience is meaningful inflation, which is backwards for this audience; (2) do NOT over-correct into a Japan-centred article either. Target = a genuinely global topic, Japan as ONE well-understood case among several (≥3 regions per Reading, no country dominant). The IR resource is the model: it uses Japanese companies freely alongside many others while keeping a global focus.
  - **D2 Book A keeps all 20 topics unchanged.** Fix the 3 defective topics by writing correct content in the rebuild.
  - **D3 Reading length = whatever fills one A4 page as a 2-column article under the Goal block, no overflow.** The IR book sets this standard; measured shipped IR Readings = 422–487 words, mean ~453, none reached 500. Working target ~440–475 words, ceiling ~490. The 450–550 in the IR QA checklist was aspirational and not held to. Real gate is the Phase-7 rendered-PDF page-fit check; no 2-column A4 reference DOCX exists yet (must be built before Phase 6/7).
  - **D4 sourcing = every factual anchor gets an inline `[N]` marker → numbered Source Note with a live, publicly reachable URL** (regulator / central bank / standard-setter / official stats agency / company IR / established news outlet — no blogs, aggregators, undated explainers). ≥2 anchors per Reading. ~40+ verified citations per book.
  - **D5 one glossary per book** (built from final Vocabulary Focus sections, IR-style). **D6** repo layout as above. **D7** old Book A material → `_archive/`. **D8** Book B's 20 topics approved as-is.
- **Paragraph-writing rule carried from the IR project** (`house-style.md`, "Paragraph writing" subsection): (1) a paragraph break must mark a genuine shift — never split one continuing idea just to shorten it; (2) NO single-sentence "moral"/"kicker" ending — the last paragraph is full and carries a substantive point.
...
## 2026-09-12 - LTF: teacher answer books drafted; Phase 5 whole-book QA audit run; textinspector.com workflow established

- Decision (teacher answer book scheduling): resolved by inspecting the IR project's own history (`IR revision audit report.md`) that a book's teacher answer book is drafted early, alongside/soon after the articles, NOT deferred to Phase 6 assembly as `PROJECT-PLAN.md` §4 could be misread to imply. Once drafted, it is kept in permanent lockstep with the articles via a standing three-way cross-check (article Vocabulary Focus <-> teacher-book Target vocabulary <-> glossary tags), re-run after every structural article edit, not a one-time deliverable. Both books' `drafts/teacher-answer-book.md` are now drafted (40 topics total; commits `1a5190a` Book A, `8a3d7f7` Book B) and mechanically verified (Aim==Goal exact match, vocab match, 5+5 answer counts, confidentiality notes, all 40 topics clean).
- Constraint/gap: no `glossary.md` and no `00_How_This_Resource_Is_Organized.md` (front matter) exist yet for either book. Both are required before the vocabulary three-way check can run in full and before the Phase 5 QA checklist's Glossary section and front-matter line can be evaluated at all. Needed regardless of anything else, before Phase 6.
- Decision (Phase 5 whole-book QA method): run the full `<book>/drafts/control/qa-checklist-full.md` (89 lines) via one background agent per book, each doing a full read of all 20 articles plus house-style.md/topic-ledger.md/vocabulary-map.md/teacher-answer-book.md, rather than sampling. Findings-only pass (no fixes applied yet) - see `project-journal.md` 2026-09-12 entry for the full defect list. Key confirmed defect: Book A `2-4_Stress_Testing_and_Risk_Management.md` and Book B `2-5_Banks_What_They_Do_and_How_They_Fail.md` both use the identical Silicon Valley Bank "$40 billion in a single day" statistic as primary opening evidence - `topic-ledger.md` flagged this as "borderline" during Batch 2 drafting but the flag was never acted on, which is itself the lesson: a ledger flag is not a fix, and must be tracked to resolution, not just noted.
- Process-gap finding, both books: `company-and-geography-audit.md` was never populated during Phase 4 drafting in either book (still the blank Phase-3 template) despite being the explicit designated guard against cross-topic evidence duplication. This is very likely why the SVB duplication above went uncaught. Preferred behavior going forward for any future multi-batch drafting project: treat an unpopulated per-batch control file as a standing red flag to check explicitly at the end of each batch, not just at final QA - an empty guard file gives false confidence that the check happened.
- Decision (textinspector.com file-upload bug workaround): textinspector.com's file-upload import path strips periods from the text on import - confirmed across `.md` and `.txt` uploads, confirmed NOT an encoding issue (verified plain UTF-8, no BOM, real ASCII period bytes present in the raw file). The only reliable path found: paste the text directly into textinspector's input box instead of uploading a file, which bypasses whatever cleaning step the upload path runs. Always use paste, not upload, for this tool until/unless the bug is confirmed fixed upstream.
- Related decision: textinspector's "Split documents at this character" field is not purely a multi-document separator - when set, it appears to ALSO become the sentence-boundary delimiter, causing the tool to ignore `.`/`?`/`!` entirely and treat each whole document as one giant "sentence" (symptom: "Sentence count" exactly equals the number of documents/articles). Leave that field empty for a single-document analysis; only use it with real caution, and verify the returned sentence count looks plausible before trusting any readability/CEFR number from that run.
- Finding: with the upload bug worked around (pasted text, no split character), textinspector's own analysis of the Reading-only prose (citation markers stripped) gives believable numbers matching independently-verified house-style checks: Book A 449 sentences / 20.8 words-per-sentence average, Book B 476 sentences / 19.6 words-per-sentence average - both inside the B1+/B2 target of 15-25 words. But textinspector's own CEFR Scorecard rates both books **C1+** overall against the **B1+/B2** target, driven by vocabulary sophistication (Academic Word List coverage ~14-15% of word types, only ~33% of word types at EVP A1 level) rather than sentence mechanics, which are on target. Read this as a real, useful independent signal about vocabulary density, not as proof the books are miscalibrated - a general-English CEFR scorer has no visibility into this project's deliberate New/Recycled term pedagogy, so some elevated academic-vocabulary share for a finance-literacy resource is expected and by design, not necessarily a defect.
- Built `books/_lets-talk-finance-shared/textinspector-known-words.txt` (349 words), `...-acronyms.txt` (13 acronyms, both cases), and `...-combined.txt`, extracted from all 40 articles' New+Recycled Vocabulary Focus lines, decomposed into individual content words with grammatical filler stripped, for use with textinspector's "known words list" upload feature - intended to stop the CEFR scorer penalizing deliberately-taught finance vocabulary, isolating the C1+ signal to genuinely incidental difficulty. Not yet re-run through textinspector with this list applied - that is the next step if this line of analysis continues.
