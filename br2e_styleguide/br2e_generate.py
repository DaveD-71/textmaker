...
{chr(10).join(f"  --fs-{e[0]}: {e[4]:.2f}px;" for e in D.TYPE_SCALE)}
...
def build_s1():
    ...
    for label, val_mm, axis, note_text in D.GEOMETRY:
    ...
    pos_rows = ''
    for label, y_top, y_bot, height, note_text in D.POSITIONS:
        y_top_s  = f'{y_top}mm'
    ...
    # Layout variants
    layouts = [
        ('Unit opening (recto)', f'Full-width photo ({D.POSITIONS[2][3]}mm); running header overlapping lower photo ({D.POSITIONS[1][3]}mm); #3C4C4C unit num block left ({D.GEOMETRY[3][1]}×{D.GEOMETRY[4][1]}mm) with 2mm level-accent stripe; Starting Point box lower-right ({D.GEOMETRY[6][1]}×{D.GEOMETRY[7][1]}mm, bg {D.COLOURS["blue_lt"]["hex"]}, top rule 3.9pt teal)'),
        ('Activity page (recto)',  f'#6C7F7F header band top ({D.GEOMETRY[0][1]}mm). Sidebar {D.GEOMETRY[8][1]}mm wide against inner margin. Main column ~121mm. Sidebar always on gutter side.'),
        ...
        ('Viewpoint lesson',       f'"Viewpoint N" News Gothic Bold {D.TYPE_SCALE[1][4]}pt white, pipe Regular {D.TYPE_SCALE[2][4]}pt {D.COLOURS["amber"]["hex"]} amber, topic Regular {D.TYPE_SCALE[2][4]}pt white; Preview sidebar; activity content main column'),
...
def build_s2():
    ...
    for code, title, key in specimens:
        entry = next((e for e in D.TYPE_SCALE if e[0]==key), None)
...
def build_s5():
    ...
    components = [
        ('01', 'Unit Header Band', 'Unit opening recto',
         ...
         f'Unit number block: {D.GEOMETRY[3][1]}×{D.GEOMETRY[4][1]}mm, bg {D.COLOURS["charcoal"]["hex"]}. '
         f'Accent stripe: {D.GEOMETRY[5][1]}mm wide, level colour. '
         f'Title: News Gothic Bold {D.TYPE_SCALE[0][4]:.2f}pt white on {D.COLOURS["hdr_band"]["hex"]}.'),
        ('02', 'Interior Running Header', 'All interior pages',
         ...
         f'Bg: {D.COLOURS["hdr_band"]["hex"]}. Text: News Gothic Bold+Regular {D.TYPE_SCALE[8][4]:.2f}pt white. '
         f'Bold for "Unit N", Regular for topic. Pipe in {D.COLOURS["amber"]["hex"]} amber. Height: {D.GEOMETRY[0][1]}mm.'),
        ('03', 'Section Header', 'WwW · LAW · BC · PS',
         ...
         f'Text: News Gothic Bold {D.TYPE_SCALE[7][4]:.2f}pt {D.COLOURS["teal"]["hex"]}. '
         f'Rule: {D.RULES[0][1]}pt solid {D.COLOURS["teal"]["hex"]}. Always at y={D.GEOMETRY[11][1]}mm from content top.'),
        ('04', 'Activity Items', 'All activity pages',
         ...
         f'Number: News Gothic Bold {D.TYPE_SCALE[8][4]:.2f}pt {D.COLOURS["charcoal"]["hex"]}. '
         f'Instruction: Palatino Medium {D.TYPE_SCALE[15][4]:.2f}pt black. Hanging 1.8mm.'),
        ('05', 'Key Expressions Box', 'Business Communication pages',
         ...
         f'Bg: {D.COLOURS["blue_lt"]["hex"]}. Width: {D.GEOMETRY[8][1]}mm. Padding: {D.GEOMETRY[9][1]}mm. '
         f'Title: News Gothic Bold {D.TYPE_SCALE[12][4]:.2f}pt {D.COLOURS["teal"]["hex"]}. '
         f'Lines: News Gothic Regular {D.TYPE_SCALE[27][4]:.2f}pt.'),
        ('06', 'Language Point Box', 'Language at Work pages',
         ...
         f'Sidebar flag: {D.COLOURS["blue_mid"]["hex"]}. Header: News Gothic Bold {D.TYPE_SCALE[18][4]:.2f}pt {D.COLOURS["slate"]["hex"]}. '
         f'Body: Palatino {D.TYPE_SCALE[15][4]:.2f}pt. Note: flag rect is in sidebar column; text is in main column.'),
        ('07', 'Cross-reference Line', 'End of activity sections',
         ...
         f'Text: News Gothic Bold {D.TYPE_SCALE[14][4]:.2f}pt {D.COLOURS["crimson"]["hex"]}. '
         f'❯❯ marker: ZapfDingbats {D.TYPE_SCALE[12][4]:.2f}pt. Border top: 0.8pt {D.COLOURS["rule_grey"]["hex"]}.'),
        ('08', 'Talking Point Band', 'Talking Point pages',
         ...
         f'Title: {D.FONTS["slab"]["oup"]}-Bold {D.TYPE_SCALE[4][4]:.2f}pt {D.COLOURS["crimson"]["hex"]}.'),
        ('09', 'Viewpoint Band', 'After every 3rd unit',
         ...
         f'Bg: {D.COLOURS["charcoal"]["hex"]}. "Viewpoint N": News Gothic Bold {D.TYPE_SCALE[1][4]:.2f}pt white. '
         f'Pipe: Regular {D.TYPE_SCALE[2][4]:.2f}pt {D.COLOURS["amber"]["hex"]} amber. '
         f'Topic: Regular {D.TYPE_SCALE[2][4]:.2f}pt white. Same size as "Viewpoint N" — weight only differs.'),
    ]
...
def build_s6():
    ...
    # Leading table
    lead_rows = ''
    for role, lead_mm, lead_pt in D.LEADING:
        if lead_mm is None:
    ...
    # Gaps table
    gap_rows = ''
    for from_to, mm_lo, mm_hi, pt_lo, pt_hi, note in D.GAPS:
    ...
    # X positions table
    xpos_rows = ''
    for label, x_recto, x_verso, note in D.X_POSITIONS:
    ...
    kv_data = [
        ('Box width', f'{D.GEOMETRY[8][1]}mm', ''),
        ('Box position', 'x=0mm recto / x=35.4mm verso (gutter side always)', ''),
        ('Internal left padding', f'{D.GEOMETRY[9][1]}mm', f'{ph(D.GEOMETRY[9][1])} H'),
        ('"Key expressions" title', f'News Gothic Bold {D.TYPE_SCALE[12][4]:.2f}pt {D.COLOURS["teal"]["hex"]}', ''),
        ('Category labels', f'News Gothic Bold {D.TYPE_SCALE[26][4]:.2f}pt {D.COLOURS["charcoal"]["hex"]}', ''),
        ('Expression lines', f'News Gothic Regular {D.TYPE_SCALE[27][4]:.2f}pt #000000', ''),
...
def build_s8():
    ...
    # Paragraph styles
    para_rows = ''
    for name, font_key, weight, size_pt, col_hex, sp_before, sp_after, other in D.WORD_STYLES:
        f = D.FONTS.get(font_key, {})
...
{level_lines}</pre>'''
    ...
    layout_data = [
        ('BR_Unit_Opening',
         f'Photo placeholder top {D.POSITIONS[2][3]}mm; {D.COLOURS["hdr_band"]["hex"]} header band overlapping lower photo ({D.POSITIONS[1][3]}mm); {D.COLOURS["charcoal"]["hex"]} unit num block left ({D.GEOMETRY[3][1]}×{D.GEOMETRY[4][1]}mm) with 2mm level-accent stripe; Starting Point box lower-right ({D.GEOMETRY[6][1]}×{D.GEOMETRY[7][1]}mm, bg {D.COLOURS["blue_lt"]["hex"]}, top rule 3.9pt teal)'),
        ('BR_Activity_Page',
         f'{D.COLOURS["hdr_band"]["hex"]} header band top; main column; Key Expressions sidebar {D.GEOMETRY[8][1]}mm wide; section header with {D.RULES[0][1]}pt teal underline'),
        ('BR_Talking_Point',
         f'Full photo top; {D.COLOURS["amber"]["hex"]} "TALKING POINT" label; {D.FONTS["slab"]["oup"]}-Bold {D.TYPE_SCALE[4][4]:.2f}pt {D.COLOURS["crimson"]["hex"]} title; Discussion/Task sub-heads News Gothic Bold {D.TYPE_SCALE[9][4]:.2f}pt {D.COLOURS["amber"]["hex"]}'),
        ('BR_Viewpoint',
         f'{D.COLOURS["charcoal"]["hex"]} bg full-width · "Viewpoint N": News Gothic Bold {D.TYPE_SCALE[1][4]:.2f}pt white · Pipe: Regular {D.TYPE_SCALE[2][4]:.2f}pt {D.COLOURS["amber"]["hex"]} amber · Topic: Regular {D.TYPE_SCALE[2][4]:.2f}pt white'),
        ('BR_Contents',
...
{nav_links}
  ...
  <span>BR2e Design System v{D.VERSION} — generated from br2e_data.py</span>
